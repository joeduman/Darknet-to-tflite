...
from tensorflow.keras.layers import (Conv2D, DepthwiseConv2D, Input, ZeroPadding2D, Add, Multiply, Lambda, Dropout,Reshape,
                          UpSampling2D, MaxPooling2D, AveragePooling2D, Dense, Flatten, Concatenate, Activation, GlobalAveragePooling2D, Lambda)
...
def swish(x): # yolov7 activation
    return x * K.sigmoid(x)

def mish(x): # yolov4 activation
...
def main(args):
    ...
    if width and height and args.fixed_input_shape:
        ...
        print(width, height, " Input Size")
    else:
        input_layer = Input(shape=(None, None, 3), name='image_input')
        print("None, None Input Size")
    ...
    for section in cfg_parser.sections():
        ...
        if section.startswith('convolutional'):
            ...
            if activation == 'linear':
            	...
            	act_layer = Activation(swish)(prev_layer)
            	prev_layer = act_layer
            	all_layers.append(act_layer)
            ...
            print(f"Route Layer: {section}, IDs: {ids}, Total Layers: {len(all_layers)}")
